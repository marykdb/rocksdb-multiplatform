...
protected constructor(internal val native: CPointer<rocksdb_env_t>?)
    ...
    actual fun getBackgroundThreads(priority: Priority): Int {
        checkOwningHandle()
        return rocksdb.rocksdb_env_get_background_threads_with_priority(native, priority.value.toInt())
    }
    ...
    actual fun getThreadPoolQueueLen(priority: Priority): Int {
        checkOwningHandle()
        return rocksdb.rocksdb_env_get_thread_pool_queue_length(native)
    }
...
actual fun getDefaultEnv(): Env = RocksEnv(
    requireNotNull(rocksdb_create_default_env()) {
        "Unable to allocate RocksDB default environment"
    }
)
